...
export function Sidebar() {
  ...
  return (
    <aside className="w-60 bg-white border-r border-slate-200 flex flex-col justify-between shrink-0 h-screen sticky top-0 text-slate-700 shadow-sm">
      {/* Top Logo & Main Nav */}
      <div>
        {/* Brand Logo */}
        <div className="h-16 flex items-center px-6 border-b border-slate-100">
          <Link href="/dashboard" className="flex items-center space-x-2.5">
            ...
            <span className="text-xl font-bold text-slate-900 tracking-tight">
              zoom<span className="text-[#0E71EB] font-normal text-xs ml-1 uppercase border border-[#0E71EB]/30 px-1.5 py-0.5 rounded bg-[#F0F7FF]">Workplace</span>
            </span>
          </Link>
        </div>
        ...
        <nav className="px-3 space-y-1">
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                <Icon className={`w-4 h-4 ${isActive ? "text-[#0E71EB]" : "text-slate-400"}`} />
                <span>{item.name}</span>
              </Link>
            );
          })}
